...
const updateTransactionByIdService = async (transactionId: string, data: Partial<TransactionDTO>) => {
...
const deleteTransactionByIdService = async (transactionId: string) => {
    return await database.delete(transactionTable).where(eq(transactionTable.id, transactionId));
}

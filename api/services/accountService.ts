...
const createAccountService = async (data: AccountDTO) => { 
    return await database.insert(accountTable).values(data)
}

const updateAccountByIdService = async (accountId: string, data: Partial<AccountDTO>) => {
    return await database.update(accountTable).set(data).where(eq(accountTable.id, accountId));
}

const deleteAccountByIdService = async (accountId: string) => {
    return await database.delete(accountTable).where(eq(accountTable.id, accountId));
}
